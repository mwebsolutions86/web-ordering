...
type Product = {
  id: string; name: string; price: number; description: string | null; image_url: string | null;
  type: 'simple' | 'variable' | 'combo'; is_available: boolean;
  ingredients: string[] | null; options_config: any; variations: ProductVariation[]
}
...
export default function MenuPage() {
 ...
 const handleAddToCart = (itemPayload: any) => {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-white"><Loader2 className="w-12 h-12 animate-spin text-gray-400" /></div>
    ...
    <div className="min-h-screen bg-gray-50 font-sans pb-32">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-12 max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-4">
               <span style={{ backgroundColor: PRIMARY, color: SECONDARY }} className="text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">Ouvert maintenant</span>
               <div className="flex items-center gap-4">
                   {store?.logo_url && <img src={store.logo_url} alt="Logo" className="w-16 h-16 md:w-20 md:h-20 rounded-2xl border-2 border-white/20 shadow-lg object-cover bg-white"/>}
                   <h1 className="text-4xl md:text-6xl font-black text-white tracking-tight">{store?.name || "Universal Eats"}<span style={{ color: PRIMARY }}>.</span></h1>
               </div>
               <p className="text-gray-300 max-w-xl text-sm md:text-base line-clamp-2">{store?.description}</p>
            ...
            <div className="hidden md:block w-full max-w-md">
                <div className="relative group">
                    ...
                    <input type="text" placeholder="Rechercher..." className="w-full h-14 pl-12 pr-4 bg-white/10 backdrop-blur-md text-white placeholder-gray-300 rounded-2xl border border-white/20 focus:bg-white focus:text-black outline-none" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
                </div>
            </div>
          </div>
        </div>
      ...
      <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-b border-gray-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4">
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-4">
                {categories.map((cat) => (
                    <button key={cat.id} onClick={() => scrollToCategory(cat.id)} style={{ backgroundColor: activeCategory === cat.id ? PRIMARY : '#F3F4F6', color: activeCategory === cat.id ? SECONDARY : '#4B5563' }} className="px-5 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-all duration-300 hover:scale-105 shadow-sm">
                        {cat.name}
                    </button>
                ))}
            </div>
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8 space-y-12 mt-4">
        {categories.map((category) => {
            ...
            return (
              <section key={category.id} id={category.id} ref={(el) => { categoryRefs.current[category.id] = el; }} className="scroll-mt-48">
                <h2 className="text-2xl md:text-3xl font-black text-gray-900 mb-6 flex items-center gap-3">{category.name}<span className="h-1 flex-1 bg-gray-100 rounded-full"></span></h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredProducts.map((product) => (
                    <div key={product.id} className="group bg-white rounded-[24px] border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer overflow-hidden flex flex-col h-full" onClick={() => { setSelectedProduct(product); setIsModalOpen(true); }}>
                      <div className="h-56 overflow-hidden relative">
                        <img src={product.image_url || 'https://via.placeholder.com/400x300'} alt={product.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                        <div className="absolute bottom-3 right-3 bg-white/90 backdrop-blur px-3 py-1 rounded-lg shadow-sm font-bold text-sm">
                            {product.type === 'variable' && product.variations.length > 0 ? `Dès ${Math.min(...product.variations.map(v => v.price))} DH` : `${product.price} DH`}
                        </div>
                      </div>
                      <div className="p-5 flex-1 flex flex-col">
                        <div className="flex-1">
                            <h3 className="font-bold text-lg text-gray-900 leading-tight transition-colors">{product.name}</h3>
      ...
      {cartCount > 0 && (
        <div className="fixed bottom-6 right-6 z-[100] animate-in slide-in-from-bottom-4 fade-in duration-500">
            <Link 
                ...
                className="flex items-center gap-4 pl-5 pr-2 py-2 rounded-full shadow-2xl hover:brightness-110 transition active:scale-95 group cursor-pointer"
            >
